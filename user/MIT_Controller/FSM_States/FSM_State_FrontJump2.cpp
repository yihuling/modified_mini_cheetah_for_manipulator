...
FSM_State_FrontJump2<T>::FSM_State_FrontJump2(
        ...
        : FSM_State<T>(_controlFSMData, FSM_StateName::FRONTJUMP2,"FRONTJUMP2"),outFile("./frontjump2_tmp.txt") {
...
void FSM_State_FrontJump2<T>::BalanceStandStep() {
    ...
    real_ace_world[0]=seResult.aWorld[0];
    real_ace_world[1]=seResult.aWorld[1];
    real_ace_world[2]=seResult.aWorld[2];
    ...
    if(_count < 1500) { //准备起跳
        ...
        for(int leg=2;leg<4;leg++) //后腿
        {
            _SetJPosInterPts(_count,1200,leg,initial_jpos[leg],fin);
            this->_data->_legController->commands[leg].forceFeedForward=ff_force;
//            std::cout<<"rear ff force: "<<ff_force<<std::endl;
        }


    } else if(_count > 1500 &&_count < 2100) { //起跳
        if (enter_once == 0) {
            for (size_t i(0); i < 4; ++i) {
                initial_jpos[i] = this->_data->_legController->datas[i].q;
                initial_Ppos[i] = this->_data->_legController->datas[i].p; //in body frame
            }
            enter_once = 1;
        }
...
//            Vec3<float> v_rear(-3.5,0,-0.8);
        Vec3<float> v_front(0.1, 0, -0);
        Vec3<float> v_rear(-0.1, 0, -0.);
        Vec3<float> pos;
        ...
        for (int leg = 0; leg < 4; ++leg) {
            ...
            this->outFile << std::fixed << std::setprecision(5) << _count << " "
                          << real_ace_world[0] * 0.002 * (_count - 1500) << " "
                          << real_ace_world[1] * 0.002 * (_count - 1500) << " "
                          << real_ace_world[2] * 0.002 * (_count - 1500) << std::endl;

            fin = InverseKinematics(pos);
            if (_count > 2098) {
                if (leg < 2) {
                    fin[1]-=0.3;
                    ff_front_fun_last = fin;
                } else {
                    ff_rear_fun_last = fin;
                }
...
//        if (enter_once == 0) {
        ...
        fin<<0,-1.2284329048795655,2.651189085435916;
        ff_force<<0,0,-200;
        ff_force=ff_force*0.0;
        for(int leg=0;leg<2;leg++) //前腿
        {
            _SetJPosInterPts(_count,2100+1200,leg,initial_jpos[leg],fin);
            this->_data->_legController->commands[leg].forceFeedForward=ff_force;
        }
...
//            if (i<2)
//            {
//                this->_data->_legController->commands[i].qDes = ff_front_fun_last;
//            }else {
//                ff_rear_fun_last[1]+=0.5;
//                this->_data->_legController->commands[i].qDes = ff_rear_fun_last;
//            }
//            this->_data->_legController->commands[i].kpJoint = kpMat;
//            this->_data->_legController->commands[i].kdJoint = kdMat;
////            initial_Ppos[i] = this->_data->_legController->datas[i].p; //in body frame
//        }

    }
//    else if(false) {// (_count < 1700) {
...
//        this->outFile << fixed << setprecision(5) <<_count<<" "<< this->_data->_legController->commands[0].qDes[0] <<" "
//        <<this->_data->_legController->commands[0].qDes[1]<<" "<<this->_data->_legController->commands[0].qDes[2]<< endl;
//
