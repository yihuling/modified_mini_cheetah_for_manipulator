...
void FSM_State_BalanceStand<T>::onEnter() {
  ...
  _body_weight = this->_data->_quadruped->_bodyMass * 9.81;//这里使用的只是上身的质量
...
FSM_StateName FSM_State_BalanceStand<T>::checkTransition() {
  ...
  switch ((int)this->_data->controlParameters->control_mode) {
    ...
    case K_FRONTJUMP2:
          ...
          break;
    case K_SITDOWN:
          this->nextStateName = FSM_StateName::SITDOWN;
          this->transitionDuration = 0.;
          break;
      case K_FRONTFLIP:
          ...
          break;
    case K_FRONTJUMPOPT:
          this->nextStateName = FSM_StateName::FRONTJUMPOPT;
          this->transitionDuration = 0.;
          break;
...
void FSM_State_BalanceStand<T>::BalanceStandStep() {
  ...
  if(this->_data->controlParameters->use_rc){
    ...
    // Orientation
    _wbc_data->pBody_RPY_des[0] = rc_cmd->rpy_des[0]*1.4;//*1.4;
...
//  std::cout<<" wbc leg0"<<this->_data->_legController->commands[0].qDes
//  <<" leg1 "<<this->_data->_legController->commands[1].qDes
//  <<" leg2 "<<this->_data->_legController->commands[2].qDes
//  <<" leg3 "<<this->_data->_legController->commands[3].qDes<<std::endl;
}
