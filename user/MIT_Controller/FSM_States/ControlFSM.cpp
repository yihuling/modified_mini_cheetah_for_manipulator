...
//  statesList.frontJump = new FSM_State_FrontJump<T>(&data);
  ...
  statesList.frontJumpOpt = new FSM_State_Jump5TypesMotionOPT<T>(&data);
//  statesList.frontflip = new FSM_State_FrontFlip<T>(&data);
  ...
  statesList.armControl = new FSM_State_ArmControl<T>(&data);
...
//        printf("K_BALANCE_STAND %.2f\t%.2f\n",data._stateEstimator->getResult().rpy(0),data._stateEstimator->getResult().rpy(1));
    ...
    } else if(rc_mode == RC_mode::FRONT_JUMP_OPT ){
//        printf("BACKFLIP\n");
      data.controlParameters->control_mode = K_FRONTJUMPOPT;
    }
//    else if(rc_mode == RC_mode::FRONT_JUMP_OPT){
////        printf("FRONT_JUMP\n");
//        data.controlParameters->control_mode = K_FRONTJUMPOPT;// K_FRONTJUMP2; //
//    }
      //data.controlParameters->control_mode = K_FRONTJUMP;
    //std::cout<< "control mode: "<<data.controlParameters->control_mode<<std::endl;
  } else
  {   //LT standup
      if(data._desiredStateCommand->returnBackGamepad()->leftTriggerAnalog){
          /*data.controlParameters->control_mode = K_RECOVERY_STAND;*/
          data.controlParameters->control_mode = K_ARMCONTROL;
      }else if(data._desiredStateCommand->returnBackGamepad()->rightTriggerAnalog){
          //data.controlParameters->control_mode = K_SITDOWN;
          data.controlParameters->control_mode = K_PASSIVE;
      }
      /*--------------------------被yihu注释掉--------------------------------------------------*/
      //else if(data._desiredStateCommand->returnBackGamepad()->leftBumper){
      //    data.controlParameters->control_mode = K_BALANCE_STAND;

      //}else if(data._desiredStateCommand->returnBackGamepad()->rightBumper){
      //    data.controlParameters->control_mode = K_PASSIVE;

      //} else if(data._desiredStateCommand->returnBackGamepad()->x){
      //  data.controlParameters->control_mode = K_LOCOMOTION;

      //} else if(data._desiredStateCommand->returnBackGamepad()->y){
      //    data.controlParameters->control_mode = K_FRONTJUMPOPT;

      //} else if(data._desiredStateCommand->returnBackGamepad()->a){

      //    data.controlParameters->control_mode = K_BACKFLIP;
      //} else if(data._desiredStateCommand->returnBackGamepad()->b){
...
//      //左侧LT按下，恢复站立状态
//      if(data._desiredStateCommand->returnBackGamepad()->leftTriggerAnalog)
//          data.userParameters->cmpc_gait=4; //
//      //右侧RT按下，PASSRIVE状态
//      if(data._desiredStateCommand->returnBackGamepad()->rightTriggerAnalog)
//          this->nextStateName = FSM_StateName::PASSIVE;
//      //左侧LB按下，Trot步态
//      if(data._desiredStateCommand->returnBackGamepad()->leftBumper)
//          data.userParameters->cmpc_gait=9;
//      //右侧RB按下，flying trot
//      if(data._desiredStateCommand->returnBackGamepad()->rightBumper)
//          data.userParameters->cmpc_gait=5;
//      // bound
//      if(data._desiredStateCommand->returnBackGamepad()->y)
//          data.userParameters->cmpc_gait=1;
//      // pronk
//      if(data._desiredStateCommand->returnBackGamepad()->a)
//          data.userParameters->cmpc_gait=2;
//      // slow trot
//      if(data._desiredStateCommand->returnBackGamepad()->b)
//          data.userParameters->cmpc_gait=3;
//      // walking
//      if(data._desiredStateCommand->returnBackGamepad()->x)
//          data.userParameters->cmpc_gait=6;
  }
...
//      return statesList.frontJump;
    ...
    case FSM_StateName::FRONTJUMPOPT:
          return statesList.frontJumpOpt;
    case FSM_StateName::SITDOWN:
          return statesList.sitDown;
    case FSM_StateName::ARMCONTROL:
        return statesList.armControl;
    default:
      return statesList.invalid;
  }
}
